#SYNTAX: python vlctest.py media_foldername
...
model.load_state_dict(torch.load("hand_gesture_v4", map_location = torch.device('cpu')))
model.eval()
...
def capture():
	...
	while(ret):
		...
		npframe = np.array(frame).reshape(1, 50, 50, 3)
